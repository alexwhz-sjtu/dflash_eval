...
class FeatureChunkZipWriter:
    ...
    def _zip_path(self, chunk_id: int) -> Path:
    ...
    def _switch_chunk(self, chunk_id: int):
        ...
        self.current_zip = zipfile.ZipFile(self._zip_path(chunk_id), mode="a", compression=zipfile.ZIP_STORED)
    ...
    def write(self, sample_idx: int, feature_pack: dict):
        ...
        self.current_zip.writestr(f"{sample_idx}.pt", buffer.getvalue())
...
def collect_data_from_target_model(
    ...
    features_dir = output_dir / f"{dataset_name}_features"
    if save_hidden_states:
        features_dir.mkdir(parents=True, exist_ok=True)
    feature_writer = FeatureChunkZipWriter(features_dir, rank, features_per_file) if save_hidden_states else None
    ...
    try:
        # 处理每个样本
        for idx in tqdm(shard_indices, disable=not is_main_process):
            example = dataset[idx]
            source_idx = int(selected_global_indices[idx]) if selected_global_indices is not None else int(idx)
            # 获取prompt（取第一轮对话）
            ...
            with torch.no_grad():
                ...
                response_ids = generated_ids[num_input_tokens:]

                # 可选：保存目标模型隐藏状态（离线训练，避免训练时跑大模型）
                if save_hidden_states:
                    # 直接复用 generate 返回的逐步 hidden states。
                    # response 部分保存的是“生成该 token 之前”的 hidden。
                    ...
                    # 这样与离线训练目标完全对齐，并显著减少存储体积。
                    align_start = max(num_input_tokens - 1, 0)
                    aligned_target_hidden = full_target_hidden[:, align_start:, :]
                    aligned_token_ids = generated_ids[align_start:]
            ...
            # 定期保存
            if (idx + 1) % 100 == 0:
                print(f"[rank {rank}] 已处理全局索引 <= {idx}")
    except Exception as e:
        write_rank_done_marker(
            done_dir=done_dir,
            dataset_name=dataset_name,
            rank=rank,
            num_processed=len(all_responses),
            status="error",
    ...
    if is_main_process:
        ...
        if save_hidden_states:
            ...
            for chunk_id in range(total_chunks):
                ...
                with zipfile.ZipFile(merged_chunk, mode="w", compression=zipfile.ZIP_STORED) as out_zip:
                    for shard_rank in range(world_size):
    ...
    if is_main_process:
        ...
        config = {
            ...
            "feature_format": "chunk_zip" if save_hidden_states else None,
...
def main():
    ...
    parser.add_argument("--features_per_file", type=int, default=10000,
                       help="每个特征文件保存的样本数量")
