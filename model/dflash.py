...
from .debug_util import SpecDebugRecorder
...
class Qwen3DFlashAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        # K, V projection: concat first, then project once (more efficient)
        # target_hidden: include all accepted tokens' hidden states last time
        kv_input = torch.cat([target_hidden, hidden_states], dim=1)  # [bsz, ctx_len + q_len, hidden_size]
...
class DFlashDraftModel(Qwen3PreTrainedModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if target_hidden is None:
            target_hidden = hidden_states.new_zeros(hidden_states.shape[0], 0, self.config.hidden_size)
        elif target_hidden.shape[1] > 0:
    ...
    def prefill_cache_from_target_hidden(
        self,
        target_hidden: torch.Tensor,
        position_ids: torch.LongTensor,
        past_key_values: Optional[Cache] = None,
    ) -> Optional[Cache]:
        if target_hidden is None or target_hidden.shape[1] == 0 or past_key_values is None:
            return past_key_values

        projected_target_hidden = self.hidden_norm(self.fc(target_hidden))
        cos, sin = self.rotary_emb(projected_target_hidden, position_ids)

        for layer in self.layers:
            key_states = layer.self_attn.k_proj(projected_target_hidden).view(
                projected_target_hidden.shape[0],
                projected_target_hidden.shape[1],
                -1,
                layer.self_attn.head_dim,
            )
            value_states = layer.self_attn.v_proj(projected_target_hidden).view(
                projected_target_hidden.shape[0],
                projected_target_hidden.shape[1],
                -1,
                layer.self_attn.head_dim,
            ...
            key_states = apply_rotary_pos_emb_to_k(key_states, cos, sin)
            ...
            past_key_values.update(
                ...
                {"sin": sin, "cos": cos, "cache_position": position_ids},
            )
    ...
    def spec_generate(
        ...
        first_sampled_token_ids = debug_recorder.token_ids_from_tensor(first_sampled_ids)
        debug_recorder.record_prefill(first_sampled_token_ids=first_sampled_token_ids)
        ...
        while start < max_length:
            ...
            context_token_ids = debug_recorder.token_ids_from_tensor(output_ids[:, :start])
            block_seed_token_ids = debug_recorder.token_ids_from_tensor(block_output_ids[:, :1])
            ...
            draft_sampled_token_ids = debug_recorder.token_ids_from_tensor(draft_sampled_ids)
            ...
            accepted_token_ids = debug_recorder.token_ids_from_tensor(block_output_ids[:, : acceptance_length + 1])
            ...
            debug_recorder.add_step(
                ...
                block_position_ids=debug_recorder.token_ids_from_tensor(block_position_ids),
                draft_sampled_token_ids=draft_sampled_token_ids,
